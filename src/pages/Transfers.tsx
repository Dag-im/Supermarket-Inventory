...
export const Transfers: React.FC = () => {
  const { products, transfers, addTransfer, fulfillTransfer, activeRole, currentUser } = useApp();
    ...
    <div className="space-y-8 max-w-[1600px] mx-auto">
      <div className="flex items-end justify-between">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 bg-indigo-50 rounded-[2rem] flex items-center justify-center text-brand-primary shadow-xl shadow-indigo-500/10">
            <Truck className="w-8 h-8" />
          </div>
          <div>
            <h1 className="text-4xl font-black tracking-tighter text-ink-main">Logistics</h1>
            <p className="text-ink-muted text-sm mt-1 font-medium">Internal stock movements and replenishment pipeline.</p>
          </div>
        </div>
        {(activeRole === UserRole.OWNER || activeRole === UserRole.DISPATCH_MANAGER) && (
          <button 
          ...
          <div className="flex gap-4 bg-white p-2 rounded-3xl border-2 border-indigo-50 w-fit shadow-xl shadow-indigo-500/5">
            <button 
              ...
              className={cn(
                ...
                activeTab === 'PENDING' ? "bg-brand-primary text-white shadow-lg shadow-indigo-500/30" : "text-ink-muted hover:bg-indigo-50"
              )}
            >
            ...
            <button 
              ...
              className={cn(
                "px-8 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center gap-2",
                activeTab === 'FULFILLED' ? "bg-brand-primary text-white shadow-lg shadow-indigo-500/30" : "text-ink-muted hover:bg-indigo-50"
              )}
            >
            ...
            <AnimatePresence mode="popLayout">
              {filteredTransfers.length === 0 ? (
                ...
                >
                  ...
                  <h3 className="text-xl font-black text-ink-main tracking-tight">No {activeTab.toLowerCase()} transfers</h3>
                  <p className="text-ink-muted text-sm mt-2 font-medium">All stock movements are currently up to date.</p>
                </motion.div>
              ) : (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-8 flex-1">
                        <div className="w-16 h-16 bg-indigo-50 rounded-[1.5rem] flex items-center justify-center text-brand-primary group-hover:bg-brand-primary group-hover:text-white transition-all shadow-sm">
                          <Package className="w-8 h-8" />
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center gap-3">
                            <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest bg-indigo-50 px-3 py-1 rounded-full">TRF-{transfer.id.slice(0, 8)}</span>
                            <span className="text-[10px] font-black text-ink-muted uppercase tracking-widest">{new Date(transfer.requestedAt).toLocaleDateString()}</span>
                          </div>
                          <h3 className="text-xl font-black text-ink-main mt-2 tracking-tight">{getProductName(transfer.productId)}</h3>
                          <p className="text-[11px] text-ink-muted font-black uppercase tracking-widest mt-1">SKU: {getProductSku(transfer.productId)}</p>
                        </div>
                      </div>

                      <div className="flex items-center gap-12 px-12 border-x border-indigo-50">
                        <div className="text-center">
                          <p className="text-[10px] font-black text-ink-muted uppercase tracking-widest mb-2">Route</p>
                          <div className="flex items-center gap-4">
                            <div className="px-4 py-2 bg-indigo-50 rounded-xl text-[10px] font-black uppercase tracking-widest text-ink-main">Store</div>
                            <ArrowRight className="w-4 h-4 text-brand-primary" />
                            <div className="px-4 py-2 bg-brand-primary rounded-xl text-[10px] font-black uppercase tracking-widest text-white shadow-lg shadow-indigo-500/20">Dispatch</div>
                          </div>
                        </div>
                        <div className="text-center">
                          <p className="text-[10px] font-black text-ink-muted uppercase tracking-widest mb-2">Quantity</p>
                          <p className="text-2xl font-black text-brand-primary tracking-tighter">{transfer.requestedQty} <span className="text-xs">Units</span></p>
                        </div>
                      </div>
                      ...
                      <div className="pl-12 flex items-center gap-6">
                        {transfer.status === 'PENDING' ? (
                          <div className="flex items-center gap-3">
                            {(activeRole === UserRole.OWNER || activeRole === UserRole.STORE_MANAGER) && (
                              <button 
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <button className="text-brand-primary text-[11px] font-black uppercase tracking-widest hover:underline">View Receipt</button>
                          </div>
                        )}
                      </div>
                    </div>
        ...
        <div className="space-y-8">
          <div className="bg-white border-2 border-indigo-50 rounded-[3rem] p-10 shadow-xl shadow-indigo-500/5">
            <h3 className="text-sm font-black uppercase tracking-[0.2em] text-ink-main mb-8 flex items-center gap-3">
              <AlertCircle className="w-5 h-5 text-brand-primary" /> Logistics Policy
            </h3>
            <div className="space-y-8">
              <div className="flex gap-5">
                ...
                <div>
                  <p className="text-xs font-black uppercase tracking-widest text-ink-main">Verification</p>
                  <p className="text-[11px] text-ink-muted mt-2 leading-relaxed font-medium">All transfers must be verified by the receiving manager upon arrival.</p>
                </div>
              </div>
              <div className="flex gap-5">
                <div className="w-10 h-10 bg-indigo-50 rounded-2xl flex items-center justify-center shrink-0">
                ...
                <div>
                  <p className="text-xs font-black uppercase tracking-widest text-ink-main">Auto-Adjustment</p>
                  <p className="text-[11px] text-ink-muted mt-2 leading-relaxed font-medium">Stock levels are adjusted automatically once fulfilled.</p>
                </div>
              </div>
              <div className="flex gap-5">
                <div className="w-10 h-10 bg-indigo-50 rounded-2xl flex items-center justify-center shrink-0">
                ...
                <div>
                  <p className="text-xs font-black uppercase tracking-widest text-ink-main">SLA Warning</p>
                  <p className="text-[11px] text-ink-muted mt-2 leading-relaxed font-medium">Pending requests older than 24h will be flagged for review.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-brand-primary rounded-[3rem] p-10 text-white shadow-2xl shadow-indigo-500/30 relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <p className="text-white/60 text-[11px] font-black uppercase tracking-widest mt-2">Logistics Support</p>
          ...
          <div className="fixed inset-0 bg-indigo-900/20 backdrop-blur-md z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="bg-brand-primary p-12 text-white relative overflow-hidden">
                ...
                <h2 className="text-4xl font-black tracking-tighter relative z-10">New Request</h2>
                <p className="text-white/60 text-[11px] font-black uppercase tracking-[0.3em] mt-2 relative z-10">Replenishment Order</p>
              </div>
              <form onSubmit={handleRequest} className="p-12 space-y-8">
                <div className="space-y-3">
                  <label className="text-[11px] font-black uppercase tracking-widest text-ink-muted ml-1">Select Product</label>
                  <select 
                ...
                <div className="space-y-3">
                  <label className="text-[11px] font-black uppercase tracking-widest text-ink-muted ml-1">Requested Quantity</label>
                  <input 
                    type="number" 
                    min="1"
                    value={requestedQty}
