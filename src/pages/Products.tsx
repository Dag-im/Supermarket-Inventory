...
export const Products: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setNewProduct({
      sku: '', barcode: '', name: '', description: '', category: '', subCategory: '', unit: '',
      costPrice: 0, sellingPrice: 0, minStockStore: 10, minStockDispatch: 5, isPerishable: false,
      defaultExpiryDays: 0, supplierId: ''
    });
  };
  ...
  return (
    <div className="space-y-8 max-w-[1600px] mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-4xl font-black tracking-tighter text-ink-main">Catalog</h1>
          <p className="text-ink-muted text-sm mt-1 font-medium">Manage your global product definitions and pricing.</p>
        </div>
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                <tr key={product.id} className="group hover:bg-indigo-50/30 transition-colors">
                  ...
                  <td className="data-table-cell">
                    <span className="badge-vibrant bg-indigo-50 text-brand-primary border-indigo-100">{product.category}</span>
                  </td>
                  <td className="data-table-cell">
                    <p className="text-sm font-black text-ink-main">{product.costPrice.toLocaleString()} ETB</p>
                  </td>
                  <td className="data-table-cell">
                    <p className="text-sm font-black text-brand-primary">{product.sellingPrice.toLocaleString()} ETB</p>
                  </td>
                  <td className="data-table-cell">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[11px] font-black text-emerald-600">{margin}%</span>
                    </div>
                  </td>
                  <td className="data-table-cell text-right">
                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                      <button className="w-9 h-9 flex items-center justify-center rounded-xl bg-white shadow-sm border border-indigo-50 text-ink-muted hover:text-brand-primary hover:border-brand-primary transition-all" title="Edit">
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button className="w-9 h-9 flex items-center justify-center rounded-xl bg-white shadow-sm border border-indigo-50 text-ink-muted hover:text-brand-primary hover:border-brand-primary transition-all" title="Barcode">
                        <Barcode className="w-4 h-4" />
                      </button>
                      <button 
                        onClick={() => deleteProduct(product.id)}
                        className="w-9 h-9 flex items-center justify-center rounded-xl bg-white shadow-sm border border-indigo-50 text-ink-muted hover:text-rose-500 hover:border-rose-500 transition-all" title="Delete"
                      >
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-indigo-900/20 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <motion.div 
              ...
              className="w-full max-w-2xl bg-white rounded-[32px] shadow-2xl overflow-hidden border-none"
            >
              <div className="bg-brand-primary p-10 text-white relative overflow-hidden">
                <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-32 -mt-32 blur-3xl" />
                <div className="flex justify-between items-start relative z-10">
                  <div>
                    <h2 className="text-3xl font-black tracking-tighter">New Product</h2>
                    <p className="text-white/60 text-[11px] font-black uppercase tracking-[0.3em] mt-2">Define global product parameters</p>
                  </div>
                  <button 
              ...
              <form onSubmit={handleSubmit} className="p-10 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[11px] font-black uppercase tracking-widest text-ink-muted ml-1">Product Name</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-2">
                    <label className="text-[11px] font-black uppercase tracking-widest text-ink-muted ml-1">SKU Code</label>
                    <input 
                      type="text" 
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[11px] font-black uppercase tracking-widest text-ink-muted ml-1">Category</label>
                    <input 
                      type="text" 
                  ...
                  <div className="space-y-2">
                    <label className="text-[11px] font-black uppercase tracking-widest text-ink-muted ml-1">Unit (e.g. kg, pcs)</label>
                    <input 
                      type="text" 
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[11px] font-black uppercase tracking-widest text-ink-muted ml-1">Cost Price (ETB)</label>
                    <input 
                      type="number" 
                  ...
                  <div className="space-y-2">
                    <label className="text-[11px] font-black uppercase tracking-widest text-ink-muted ml-1">Selling Price (ETB)</label>
                    <input 
                      type="number" 
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-black uppercase tracking-widest text-ink-muted ml-1">Supplier</label>
                  <select 
