...
export const SalesDashboard: React.FC = () => {
  ...
  const stats = [
    ...
    {
      name: 'Avg. Ticket',
      value: `${avgOrderValue.toFixed(0)} ETB`,
      icon: Package,
    ...
    <div className="space-y-8 max-w-400 mx-auto">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-ink-muted text-sm mt-1 font-medium">
            Real-time transaction monitoring and revenue analytics.
          </p>
        </div>
        <div className="flex gap-4">
          <div className="flex bg-indigo-50/50 border-2 border-indigo-100 rounded-2xl p-1 shadow-sm">
            <button className="px-6 py-2 text-[10px] font-black uppercase bg-white text-brand-primary rounded-xl shadow-sm transition-all">
              Today
            </button>
            <button className="px-6 py-2 text-[10px] font-black uppercase text-ink-muted hover:text-brand-primary transition-all">
              7D
            </button>
            <button className="px-6 py-2 text-[10px] font-black uppercase text-ink-muted hover:text-brand-primary transition-all">
              30D
          ...
          >
            <div className="flex items-start justify-between">
              <div
                className={cn(
                  stat.bg,
                  'p-4 rounded-2xl border border-indigo-50 group-hover:scale-110 transition-transform shadow-sm'
                )}
              >
            ...
            <div className="mt-6">
              ...
              <p className="text-2xl font-black mt-1 tracking-tighter text-ink-main">
                {stat.value}
              </p>
            </div>
          </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="card-vibrant p-8">
          <h2 className="text-[11px] font-black uppercase tracking-[0.2em] text-brand-primary mb-8">
            Top SKUs
          </h2>
          <div className="space-y-4">
            {TOP_PRODUCTS.map((product, i) => (
              <div
                key={product.name}
                className="flex items-center justify-between p-4 bg-indigo-50/30 rounded-2xl border-2 border-transparent hover:border-indigo-100 transition-all group"
              >
        ...
        <div className="p-8 bg-indigo-50/30 flex items-center justify-between border-b border-indigo-50">
          <div>
            ...
            <p className="text-[10px] text-ink-muted font-black uppercase tracking-widest mt-1">
              Live ledger of processed sales
            </p>
          </div>
          <span className="px-4 py-1.5 bg-emerald-500 text-white rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg shadow-emerald-500/30 animate-pulse">
            Live
          </span>
        </div>
            ...
            <tbody className="divide-y divide-indigo-50">
              {sales.length === 0 ? (
                <tr>
                  <td
                    colSpan={6}
                    className="px-8 py-16 text-center text-ink-muted uppercase tracking-[0.3em] font-black text-xs"
                  >
                ...
                sales.slice(0, 10).map((sale) => (
                  ...
                  >
                    <td className="data-table-cell">
                      <p className="text-[10px] font-black font-mono text-ink-muted">
                        #TXN-{sale.id.slice(0, 8).toUpperCase()}
                      </p>
                    </td>
                    <td className="data-table-cell">
                      <div className="flex items-center gap-4">
                    ...
                    <td className="data-table-cell">
                      <p className="text-sm font-black text-emerald-600">
                        {sale.revenue.toLocaleString()} ETB
                      </p>
                    </td>
                    <td className="data-table-cell">
                      <div className="flex items-center gap-2">
